 ...
 *  - {rootRef, lines}:跳表找到仍 connected 的最近祖先,局部 tree 已生成,agent 用新 ref 重试。
 *  - null:登记过但整条祖先链都已 detached(页面整体刷新/重建),提示重新 tree。
 ...
 * 判定逻辑(纯)在 find-root.ts 的 classifyRef,可单测;DOM 部分(buildTree)在此。 */
