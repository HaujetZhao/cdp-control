...
2. **端口绑定但探测期间变成健康 CDP**（另一个并发 `cdp-control` 正在拉起）→ 复用，绝不 kill。
...
| PR#2 启动中等待/复探 | 由有界重复探活、端口状态和 listener 快照复核替代；任一轮变健康都直接复用 |
| PR#2 `writeConfigAtomic` pid 后缀 | 保留（并发回写互踩修复，与本改动正交） |
| PR#1 #3 错误提示 | 保留并扩充枚举、状态、kill、释放和启动真因；不再把未知状态当空闲 |

## 已拍板的边界

1. **并发启动守卫**：破坏性操作前重复探活，并在最后一次探活后重新核对 listener PID；
...
- `src/browser.ts`：注入探活、connect/bind 状态、listener 枚举、kill、launch、sleep；所有未知
  和失败分支 fail closed。无配置时显式把 transport 恢复到 9222，不继承 `CDP_PORT` 漂移。
- `killBrowser` 复用同一 listener 枚举、kill 和端口状态原语；不再把工具失败误报成已释放。
- `tests/browser-port.test.ts`：覆盖健康复用、同端口启动、多 PID、解析过滤、双栈 wildcard、
