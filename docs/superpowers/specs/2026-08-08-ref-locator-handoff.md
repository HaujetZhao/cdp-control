...
本 skill 的定位是**服务 agent 高效读网页**。感知靠 `view`(唯一感知命令,递归穿透所有 shadow DOM),定位靠 `xpath`。
...
**ref 句柄的目标**:砍掉"agent 手写 XPath"这一层,让 agent 从 view 输出直接拿到可操作句柄,零定位负担。

---

## 2. 核心洞察

`view` **已经**递归穿透所有 shadow 并输出了完整结构。也就是说,"穿透 shadow"这件事浏览器做不了、但 view 已经做到了。

那就不该让 agent 在穿透后的世界里用 XPath 二次定位。**改为:view 遍历时给每个可操作节点登记一个引用(ref),输出标注,agent 直接操作 ref。**
...
1. **登记**:`view` 遍历整页(含 shadow)时,把每个"可操作节点"push 进页面全局数组 `window.__cdpRefs`,记下它的 index。
2. **标注**:view 输出里,该节点对应文本项追加标注 `[ref=i]`。
3. **操作**:agent 看到 `[ref=i]` 直接说"操作 ref=i",skill 用 `window.__cdpRefs[i]` 拿**真实元素引用**执行 click/fill/读取——绕开选择器字符串对 shadow 的所有限制。

### 3.2 什么算"可操作节点"(粒度)

参考 `view.ts` 现有的 `simplify()` 判断,候选:
...
- SKILL.md 已强调"自动化写成脚本一次执行"。所以 **同一脚本回合内 view(拿 ref)→ 操作** 是稳的。
- 跨命令/刷新/动态加载后 ref 序号漂移,是**预期**行为,文档要写清楚,agent 应"每回合先 view 再操作"。
...
| 场景 | 用 |
|---|---|
| 交互式点选(感知 view → 定位 → 操作单个) | **ref**(零 XPath) |
...
### 4.1 `src/inject/view.ts` — 登记 ref

- `simplify()` 构建 `ViewNode` 时,对满足"可操作"条件的节点(见 3.2):
  - `const ref = __cdpRefs.length; __cdpRefs.push(el); node.ref = ref;`
  - 在入口处先 `if (!window.__cdpRefs) window.__cdpRefs = [];`(幂等,防止重复注入时重登)。
- `ViewNode` 类型加 `ref?: number` 字段(定义在 `view-format.ts`)。
- ⚠️ 只对**真实可定位/可操作**节点登记。注意 `simplify` 遍历的是 `Element | ShadowRoot`,`ShadowRoot` 不能登记(它没有操作意义),只登记 `Element`。

### 4.2 `src/inject/lib/view-format.ts` — 输出标注

- `formatView()` 在输出文本项时,若节点有 `ref`,追加 `[ref=i]`(如 `p "评论文本" [ref=12]`)。
...
1. 在 B 站(或含大量 shadow 的页面)实测:`view` 输出某评论文本项带 `[ref=i]`。
2. `click`/`fill` 支持 `ref=i`,能对该真实元素生效——**含 shadow 内元素**(之前 click 的 CSS selector 穿不透 shadow)。
3. agent 走通"view → 看到 [ref=i] → 操作 ref=i"全程**零 XPath**。
...
- **浏览器实测**:按项目约定,注入侧 DOM 逻辑靠浏览器实测验收(CDP 端口 9222),不写单测;纯函数(如 formatView 的 ref 标注、arg 解析)可单测。
...
- **ref 序号漂移**:动态加载/刷新后 `__cdpRefs` 重置或序号变化。文档写清"每回合先 view 再操作",不做跨回合承诺。
