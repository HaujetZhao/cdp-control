# 设计：操作后自动反馈（含 tab 变化）+ view 可见性标签
...
1. **操作后不知道页面变了什么**：点"显示评论"后新增了内容、点赞后数字变了，agent 不知道，得额外多轮去 `view` 复看。
2. **点链接新开 tab 后不知道开了哪个**：很多站卡片是 `target=_blank`，点完原 tab 的 `location` 没变，agent 得额外 `list` 才能确认落点。

**目标**：一次操作命令，返回「页面新增内容 view + tab 变化摘要」，让 agent 一轮拿到结果，不再需要主动 `view`/`list` 补查。

另外：view 增加"在视区可见"标记，让 agent 知道目标元素在不在屏上、要不要先滚动。

---

## 一、view 可见性标签（A）

- 给 view 里带 ref 的节点加"在视区"标记，只标记**可见的小部分**（token 开销小）。
- 格式：在视区可见 → `[ref=53·屏]`；否则 → `[ref=53]`。
- 用途：agent 看到 `[ref=N]` 未标 `·屏`，就知道它不在当前屏幕、要先 `view --scroll-to-load` 或滚动后再操作（很多懒加载内容只有滚动到才出现/才可操作）。
- 实现：
  - `ViewNode` 加 `view?: boolean`（见 `lib/view-format.ts`）。
  ...
  - `formatView` 的 `refTag`：`node.view === true ? ' [ref='+n.ref+'·屏]' : ' [ref='+n.ref+']'`。
  - 独立于 visible-only（visible-only 的裁剪仍用更严格的 `isInView` + prune，不相干）。

## 二、ref 语义澄清（B）

- 现状（**确认不改**）：每次 `view` 清空 `window.__cdpRefs` 从 0 重建；局部 `view --ref X --ancestor k` 出来的 ref 是 0、1、2…，**会顶掉打开页面时的全局 ref**。
- 文档讲清即可：局部 view 后想再按全局 ref 操作，需重新整页 tree。反馈树也是独立的（ref 从 0 重排），agent 用反馈树里的 ref 操作新增内容。

## 三、建树 core 抽取（前置重构）

反馈要复用 view 的建树，须把 view.ts 的 DOM 采集逻辑抽成共享模块。

- 新建 `src/inject/lib/view-core.ts`，把 view.ts 里的 `DROP / strip / ownText / grabText / childrenOf / interactive / isInView / isInViewport / prune / simplify` 抽进去。
- 导出：
  ```ts
  export interface ViewBuildOpts { visibleOnly?: boolean; viewport?: boolean }
...
- `view.ts` 改成：`buildView(root, { visibleOnly, viewport: true })` + `markText` + `formatView`。
- `lib/view-format.ts`：`ViewNode` 加 `view?: boolean`；`refTag` 渲染 `·屏`。
...
- **view 全部新增内容块**（用户选定）：对每个顶层新增元素 `formatView(buildView(el, {viewport:true}))`，拼接。跳过错过的空/纯属性变化（characterData/attributes 只进摘要）。
...
- 操作 + 等待之后**再次 list**，diff：
  ...
  - `closed`：本次消失的 tab。
- 目的：agent 点完链接直接知道"新开了哪个 tab、开了什么页"，可直接去 `view --target <新tab>`，不必先 `list`。
...
| 文件 | 改动 |
|---|---|
| `src/inject/lib/view-core.ts` | 新：从 view.ts 抽 buildView + isInViewport |
| `src/inject/lib/view-format.ts` | ViewNode 加 `view`；refTag 渲染 `·屏` |
| `src/inject/lib/feedback.ts` | 新：装/收 MutationObserver + 新增内容 view + 摘要 |
| `src/inject/view.ts` | 改用 view-core + viewport 标记 |
...
- 单测（纯逻辑）：`view-format` 的 `·屏` 渲染（view 分支）；新增内容 view 拼接（若抽成纯函数）。
- 浏览器验收：
  1. `view` 整页：在视口的 ref 带 `·屏`。
  2. 知乎 `click` 显示评论 → 等 1s 自动 view 出新增评论 + 摘要。
  3. `click` 点开新 tab 的链接 → feedback.tabs.opened 含新 tab（title/url），agent 直接去操作。
  4. `press-key` 滚动 → 反馈懒加载内容。
  5. `--no-feedback` 关闭，动作照旧不等待。
  6. 回归：普通 view/click/logs 在 async footer 下正常。
