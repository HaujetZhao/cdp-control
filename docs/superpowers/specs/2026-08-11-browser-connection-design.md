...
- **端口固定 9222**（`CDP_PORT` env 仍可覆盖；transport 的 `PORT` 保持模块级常量）。
- **"保证可用"= 未启动时自动探查并自动启动**：
...
- **作用**：保存浏览器启动命令与参数。内容（JSON）：
  ```json
  { "exe": "<浏览器绝对路径>", "kind": "edge", "args": ["--remote-allow-origins=*", "--no-first-run", "--no-default-browser-check", "--disable-background-networking", "--disable-component-update", "--window-size=1600,1000"] }
  ```
- **缺失** → 自动发现首个可用浏览器，用默认参数集写此文件，再用它启动。
- **存在且合法** → 用它启动，**不再发现**（配置即权威，用户钉死了浏览器/exe/参数）。
- **坏了** → 打印清晰警告（JSON 解析失败 / `exe` 缺失或非字符串 / `exe` 文件不存在 / `args` 存在但非数组），**不 fallback 到发现**，报错让用户改文件。
- **用户可改**：改 `exe` 指向特定浏览器/安装，改 `args` 加/删参数（如受限环境加 `--no-sandbox`）。
- 运行时必选项 `--remote-debugging-port` 与 `--user-data-dir` 由工具注入，**不进文件**（用户改它们会破坏 CDP，属受保护项）。
- 原子写（tmp + rename）。
...
- **ready 路径**：1 次 GET `/json/version` → 有 `webSocketDebuggerUrl` 即 ready，返回（一次拿到 Browser 字段，合并原两次 GET）。
- **冷启动路径**：
  1. 读配置 `browser.json`：
     - 缺失 → discover 候选逐个尝试，首个能拉起者把 `{exe, kind, args:默认集}` 原子写配置，用该 exe 继续。
     - 存在且合法 → 用其 `exe`+`args`。
     - 坏 → 警告 + 报错，不兜底。
  2. `spawn(exe, [...args, --remote-debugging-port=9222, --user-data-dir=<dir>], { detached:true, stdio:'ignore' }).unref()`。
...
- **userDataDir**：`~/.cdp-control/user-data`（独立 profile，DESIGN「独立用户数据目录」），工具注入。仅冷启动自启时用到；已连接态用 9222 上现成的。
...
### D6 daemon（monitor.ts 基本不变）

- daemon 用 transport 的 `PORT`（固定 9222 / CDP_PORT），端口不变故无需端口自愈逻辑。浏览器崩溃重开后仍在同一端口，daemon 每 500ms 重扫 `/json/list` 自动重附着；浏览器长期不在则看门狗自退，下次冷启动重拉。**本设计下 daemon 无需改**，仅确认不因 D3 移除 url 分支而受影响。
