...
(() => {
  ...
  for (const el of all) {
    ...
    const text = (el.innerText || el.value || el.getAttribute('aria-label') || el.title || el.placeholder || '').replace(/\\s+/g, ' ').trim().slice(0, 120);
  ...
  if (window.__CDP_OC !== false) {
    for (let i = 0; i < out.length; i++) {
      for (let j = 0; j < i; j++) { // j 在 i 之上(z 更高或 DOM 更靠后)
        if (covers(out[j], out[i]) && out[j].text !== out[i].text) { out[i].occluded = true; break; }
      }
    }
  }
...
module.exports = {
  GEN_SEL, SNAPSHOT_JS, VIEW_JS, FIND_EL, CLICK_JS, FILL_JS, FOCUS_JS,
