...
// rootExpr:解析出建树根元素的 JS 表达式(如 `document.querySelector('#main')` / xpath 求值);
//   缺省为 document.body。可选,用于只建指定区域。注意:不能有模板字符串反引号(会被注入/读到页面里)。
function buildTreeExpr(rootExpr) {
  return `(() => {
  const root = ${rootExpr || 'document.body'};
  if (!root || root.nodeType !== 1) return { ok: false, err: '未找到匹配的根节点(selector/xpath 未命中)' };
