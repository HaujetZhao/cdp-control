...
const VISIBLE_JS = `(() => {
  ...
  // 4) 只留有内容的内容组:含标题(卡片/文章标题) 或 文字量大(文章正文);丢弃无标题短碎片(actions/author/searchbar)
  ...
  // 丢弃"包含另一个内容组"的(如整页容器/文章外层包装),保留最内层内容组(卡片/标题/正文)
  const inner = content.filter(a => !content.some(b => b !== a && a.contains(b)));
  // 5) 阅读序 + 抽取字段
