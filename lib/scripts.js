...
// 区域索引(默认 view):归纳"有 class/id 且含 ≥3 叶子块"的结构容器,取最外层大容器,按阅读序排。
// 每项:label/tag/id/class/块数/z/rect/selector——agent 读了选一个区域,再用 tree/--in/--out 深入。
// 注意:不能有模板字符串反引号(会被注入/读到页面里)。
const REGIONS_JS = GEN_SEL + `
(() => {
  const IW = window.innerWidth, IH = window.innerHeight;
  const px = Math.round(IW * 0.15), py = Math.round(IH * 0.15);
  const vp = { l: -px, t: -py, r: IW + px, b: IH + py };
  const INTERACTIVE = 'a, button, input, textarea, select, summary, [role=button], [role=link], [role=checkbox], [role=radio], [onclick], [tabindex]';
  const visible = (el) => { const cs = getComputedStyle(el); return cs.display !== 'none' && cs.visibility !== 'hidden' && parseFloat(cs.opacity) > 0; };
  function effZ(el) { let z = 0, cur = el; while (cur && cur.nodeType === 1) { const cs = getComputedStyle(cur); if (cs.position !== 'static' && cs.zIndex !== 'auto') z += parseInt(cs.zIndex, 10) || 0; cur = cur.parentElement; } return z; }
  const directText = (el) => { for (let i = 0; i < el.childNodes.length; i++) { const n = el.childNodes[i]; if (n.nodeType === 3 && (n.nodeValue || '').trim()) return true; } return false; };
  const isImg = (el) => el.tagName === 'IMG' && !/^data:/.test(el.src || '') && (el.width || 0) > 16 && (el.height || 0) > 16;
  const strip = (s) => (s || '').replace(/[\\u200B\\u200C\\u200D\\u2060\\uFEFF\\s]+/g, ' ').trim();
  const isJunkClass = (c) => !c || /^css-[A-Za-z0-9]+$/.test(c) || /^[A-Za-z0-9]{6,}$/.test(c);
  const goodClass = (el) => { const raw = (el.getAttribute && el.getAttribute('class')) || ''; return raw.split(/\\s+/).filter(c => !isJunkClass(c)).slice(0, 2); };
  const ownText = (el) => { let t = ''; for (const n of el.childNodes) if (n.nodeType === 3) t += n.nodeValue; return strip(t); };
  // 区域聚焦过滤器(--in/--out,由 api.view 前置设置)
  const IN_SEL = window.__CDP_IN || '', OUT_SELS = window.__CDP_OUT || [];
  const inRoot = IN_SEL ? (document.querySelector(IN_SEL) || null) : null;
  const outRoots = OUT_SELS.map(s => document.querySelector(s)).filter(Boolean);
  const inRegion = (el) => inRoot ? inRoot.contains(el) : true;
  ...
  for (const el of document.querySelectorAll('*')) {
    if (seen.has(el)) continue; seen.add(el);
    if (el.nodeType !== 1 || !visible(el) || !inRegion(el) || inOut(el)) continue;
    const interactive = el.matches(INTERACTIVE), isInput = el.tagName === 'INPUT' || el.tagName === 'TEXTAREA';
    if (!interactive && !isInput && !isImg(el) && !directText(el)) continue;
    const r = el.getBoundingClientRect();
    if (r.width === 0 || r.height === 0 || !(r.right > vp.l && r.bottom > vp.t && r.left < vp.r && r.top < vp.b)) continue;
    const text = (el.innerText || el.value || el.getAttribute('aria-label') || el.title || el.placeholder || '').replace(/[\\u200B\\u200C\\u200D\\u2060\\uFEFF]+/g, '').replace(/\\s+/g, ' ').trim();
    if (!text && !isImg(el) && !isInput) continue;
    leaves.push(el);
  }
  // 2) 祖先计数:每个叶子向上给带 class/id 的祖先容器 +1
  const cnt = new Map();
  for (const lf of leaves) {
    let cur = lf.parentElement;
    while (cur && cur.nodeType === 1 && cur !== document.body && cur !== document.documentElement) {
      if ((cur.id || cur.getAttribute('class')) && visible(cur)) cnt.set(cur, (cnt.get(cur) || 0) + 1);
      cur = cur.parentElement;
    }
  }
  // 3) 候选区域:排除"页面壳"(body/html/#root/#app);块数≥3;须有"语义"id/class(滤掉 css-* 哈希 class)。
  const SHELL_ID = new Set(['root', 'app', '__next', 'app-root', 'page']);
  const isShell = (el) => el === document.body || el === document.documentElement || SHELL_ID.has(el.id || '');
  const hasSemantic = (el) => el.id || (el.getAttribute('class') || '').split(/\\s+/).some(c => c && !isJunkClass(c));
  const cands = [];
  for (const [el, n] of cnt) { if (!isShell(el) && n >= 3 && hasSemantic(el)) cands.push({ el, n }); }
  // 列表容器识别:某候选的子候选 ≥2 → 保留父、压制子(评论区=1区域,而非N条评论);=0 叶子区域;=1 纯包装(下钻)
  const kidsOf = new Map();   // 父候选 → 子候选[]
  const parentOf = new Map(); // 候选 → 最近候选祖先
  for (const c of cands) {
    let p = c.el.parentElement;
    while (p && p !== document.body) {
      const pc = cands.find(x => x.el === p);
      if (pc) { parentOf.set(c, pc); break; }
      p = p.parentElement;
    }
    const par = parentOf.get(c);
    if (par) { if (!kidsOf.has(par)) kidsOf.set(par, []); kidsOf.get(par).push(c); }
  }
  const chosen = [];
  const covered = new Set();
  const walk = (c) => {
    if (covered.has(c.el)) return;
    const kids = kidsOf.get(c) || [];
    if (kids.length >= 2) { chosen.push(c); covered.add(c.el); kids.forEach(k => covered.add(k.el)); }
    else if (kids.length === 0) { chosen.push(c); covered.add(c.el); }
    else { covered.add(c.el); kids.forEach(walk); }
  };
  cands.filter(c => !parentOf.has(c)).forEach(walk); // 从最浅(无候选祖先)开始
  cands.forEach(c => { if (!covered.has(c.el)) chosen.push(c); }); // 兜底:未覆盖的孤立候选
  chosen.forEach(c => { c.z = effZ(c.el); });
  chosen.sort((a, b) => a.el.getBoundingClientRect().top - b.el.getBoundingClientRect().top); // 阅读序
  // 4) 标签:自身直接文本 → 首个子标题/正文 → class 名
  const label = (el) => {
    let t = ownText(el);
    if (!t) { const f = el.querySelector('p, h1, h2, h3, [class*=title], [class*=content]'); t = f ? strip(f.innerText || '') : ''; }
    return (t || (el.getAttribute('class') || '').split(/\\s+/)[0] || '').slice(0, 24);
  };
  return {
    viewport: { w: IW, h: IH },
    regions: chosen.slice(0, 30).map(c => {
      const r = c.el.getBoundingClientRect();
      return { label: label(c.el), tag: c.el.tagName.toLowerCase(), id: c.el.id || undefined, cls: goodClass(c.el), blocks: c.n, z: c.z, rect: { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) }, selector: genSel(c.el) };
    }),
  };
})()`;
...
// 注意:不能有模板字符串反引号(会被注入/读到页面里)。
function buildTreeExpr(sel, maxDepth = 8, maxClass = 2, out = [], vp = true) {
  return `(() => {
  ...
  const inVp = (el) => { if (!VP) return true; const r = el.getBoundingClientRect(); return r.right >= 0 && r.bottom >= 0 && r.left <= window.innerWidth && r.top <= window.innerHeight; };
...
module.exports = {
  GEN_SEL, SNAPSHOT_JS, VIEW_JS, REGIONS_JS, buildTreeExpr, FIND_EL, CLICK_JS, FILL_JS, FOCUS_JS,
