...
function buildTreeExpr(sel, maxDepth = 8, maxClass = 2, out = [], vp = true, vm = 3, vis = false) {
  return `(() => {
  ...
  const collapse = (el) => {
    let cur = el;
    while (true) {
      const kids = childrenOf(cur).filter(c => !DROP.has(c.tagName) && visible(c));
      if (kids.length === 1 && !ownText(cur) && !cur.id && goodClass(cur).length === 0
          && cur.tagName !== 'IMG' && childrenOf(kids[0]).length > 0) { cur = kids[0]; continue; }
      break;
    }
    return cur;
  };
  ...
  const walk = (el, depth) => {
    const c = collapse(el);
    const kids = childrenOf(c).filter(k => !DROP.has(k.tagName) && visible(k) && !isOut(k) && inVp(k));
    const tag = c.tagName.toLowerCase();
    const t = ownText(c);
    const cls = goodClass(c);
    // 只打印"有意义"节点(有文本/img/有id/有语义class);纯包装 div 不打印,只下钻(省上下文)
    if (t || tag === 'img' || c.id || cls.length) {
      let label = tag;
      if (c.id) label += '#' + c.id;
      if (cls.length) label += '.' + cls.join('.');
      const line = '  '.repeat(depth) + label;
      if (tag === 'img') out.push(line + (c.getAttribute('alt') ? ' "' + c.getAttribute('alt').slice(0, 40) + '"' : ''));
...
const VISIBLE_JS = `(() => {
  ...
  const strip = (s) => (s || '').replace(/[\\s\\u200B\\u200C\\u200D\\u2060\\uFEFF]+/g, ' ').trim();
  const VM = 3 * IH; // 纵向余量:上下各 3 个视口高(默认)
  const inVp = (r) => r.width > 0 && r.height > 0 && r.bottom > -VM && r.top < IH + VM && r.right > 0 && r.left < IW;
  const directText = (el) => { for (const n of el.childNodes) if (n.nodeType === 3 && (n.nodeValue || '').trim()) return true; return false; };
  const DROP = new Set(['SCRIPT', 'STYLE', 'LINK', 'META', 'NOSCRIPT', 'TEMPLATE']);
  // 泛化:递归穿透所有 shadowRoot,收集全部元素(light + shadow)
  const allEls = (() => { const out = []; (function walk(root) { for (const el of root.querySelectorAll('*')) { out.push(el); if (el.shadowRoot) walk(el.shadowRoot); } })(document); return out; })();
  // 1) 视口内叶子块(文本/可交互/图,含 shadow DOM)
  const leaves = [];
  for (const el of allEls) {
    if (el.nodeType !== 1 || !visible(el) || DROP.has(el.tagName)) continue;
    const r = el.getBoundingClientRect();
    if (!inVp(r)) continue;
  ...
  // 4) 内容组:含标题(卡片/文章标题/视频 img alt) 或 文字量大(文章正文);丢弃无标题短碎片(actions/author/searchbar)
  const hasTitle = (el) => {
    const t = el.querySelector('h1, h2, h3, a[class*=title], [class*=title]');
    if (t && !!strip(t.innerText)) return true;
    const img = el.querySelector('img[alt]'); // 视频站标题常在封面 img alt
    return !!(img && strip(img.getAttribute('alt') || '').length > 4);
  };
  const content = [...groups.values()].filter(el => hasTitle(el) || strip(el.innerText).length > 80);
  // 5) 丢"列表父/外层包装"(含 ≥2 个内容组的,如整页容器/文章外层):Topstory-container、Post-content
  const noWrap = content.filter(a => content.filter(b => b !== a && a.contains(b)).length < 2);
  // 6) 丢"被另一个内容组包含的碎片"(卡片内的正文 RichContent-inner):保留卡片 Feed、丢正文碎片
  const kept = noWrap.filter(a => !noWrap.some(b => b !== a && b.contains(a)));
  // 7) 阅读序 + 抽取字段
  const arr = kept.sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top);
  ...
  return {
    items: arr.slice(0, 40).map(el => {
      ...
      const title = firstText(el, 'h1, h2, h3, a[class*=title], [class*=title]')
        || (() => { const im = el.querySelector('img[alt]'); return im ? strip(im.getAttribute('alt') || '') : ''; })()
        || strip(el.getAttribute('class') || '').split(/\\s+/)[0] || el.tagName.toLowerCase();
