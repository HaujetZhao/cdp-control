 ...
 * 感知导向——结构 + 堆叠 + 视口,补强 snapshot(操作导向)。opts.occluded=false 可关遮挡标注。
 */
async function view(target, opts = {}) {
  const flag = opts.occluded === false ? 'window.__CDP_OC = false;' : 'window.__CDP_OC = true;';
  return evaluate(target, flag + VIEW_JS, 30000);
}

/**
 * 子树钻取:把 target 页面匹配 selector 的元素 DOM 子树输出为紧凑层级树(过滤垃圾节点/属性)。
 * 感知导向——从 view 拿到短 selector 后,钻取看某块内部的层级结构。opts:{depth(默认8), maxClass(默认2)}。
 */
async function tree(target, selector, opts = {}) {
  const r = await evaluate(target, buildTreeExpr(selector, opts.depth, opts.maxClass), 30000);
