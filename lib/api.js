...
const { SNAPSHOT_JS, VIEW_JS, buildTreeExpr, CLICK_JS, FILL_JS, FOCUS_JS, GET_FOCUS_JS, OUTLINE_JS, CONTENT_JS } = require('./scripts');
...
/**
 * 提取 target 页面"几何视图":视口内叶子文本块/可交互/图片清单,按堆叠(有效 z + DOM 序)从顶到底排序。
 * 感知导向——结构 + 堆叠 + 视口,补强 snapshot(操作导向)。
 * opts:{in 只看某容器内(阅读序), out 排除区域, occluded=false 关遮挡标注}。
 */
async function view(target, opts = {}) {
  ...
  return evaluate(target, flag + VIEW_JS, 30000);
}

/**
 * 子树钻取:把 target 页面匹配 selector 的元素 DOM 子树输出为紧凑层级树(过滤垃圾节点/属性)。
 * 感知导向——从 view 拿到短 selector 后,钻取看某块内部的层级结构。opts:{depth(默认8), maxClass(默认2), out 排除区域}。
 */
async function tree(target, selector, opts = {}) {
  const r = await evaluate(target, buildTreeExpr(selector, opts.depth, opts.maxClass, opts.out), 30000);
