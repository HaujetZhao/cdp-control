...
const { SNAPSHOT_JS, VIEW_JS, REGIONS_JS, buildTreeExpr, buildAtExpr, CLICK_JS, FILL_JS, FOCUS_JS, GET_FOCUS_JS, OUTLINE_JS, CONTENT_JS } = require('./scripts');
...
/**
 * view 区域索引(默认):归纳"有 class/id 且含 ≥3 叶子块"的主要结构容器,按阅读序排,每项带短 label+块数+z+selector。
 * 感知导航器——agent 读了选一个区域,再用 tree/--in/--out 深入。opts.all=true 返回全量叶子块(VIEW_JS)。
 * opts:{in 只看某容器内, out 排除区域, occluded=false 关遮挡标注, all 全量叶子块}。
 */
async function view(target, opts = {}) {
  const o = Array.isArray(opts.out) ? opts.out : (opts.out ? [opts.out] : []);
  const flag = `window.__CDP_IN = ${JSON.stringify(opts.in || '')};`
    + `window.__CDP_OUT = ${JSON.stringify(o)};`
    + (opts.occluded === false ? 'window.__CDP_OC = false;' : 'window.__CDP_OC = true;');
  const expr = opts.all ? VIEW_JS : REGIONS_JS;
  return evaluate(target, flag + expr, 30000);
}

/**
 * 子树钻取:把 target 页面匹配 selector 的元素 DOM 子树输出为紧凑层级树(过滤垃圾节点/属性)。
 * 感知导向——从 view 拿到短 selector 后,钻取看某块内部的层级结构。
 * opts:{depth(默认8), maxClass(默认2), out 排除区域, vp 视口过滤(默认true,只对视口内建树,false=全部),
 *       vm 纵向余量(默认1,上下各拓宽 N 个视口高,可看到折页外一屏)}。
 */
async function tree(target, selector, opts = {}) {
  ...
  const r = await evaluate(target, buildTreeExpr(sel, opts.depth, opts.maxClass, opts.out, opts.vp, opts.vm, opts.vis), 30000);
...
const coreApi = {
  ...
  snapshot, view, tree, click, fill, waitFor, waitForFn, shot, focus, getFocus, outline, content, pressKey, hover,
};
