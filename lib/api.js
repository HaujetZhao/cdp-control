...
const { SNAPSHOT_JS, buildTreeExpr, buildAtExpr, VISIBLE_JS, CLICK_JS, FILL_JS, FOCUS_JS, GET_FOCUS_JS, OUTLINE_JS, CONTENT_JS } = require('./scripts');
 ...
 * 感知主命令——唯一感知入口。
 * 无 selector 也无 --at 时:树整页可见结构(body,视口过滤,默认 depth 3,给页面全貌)。
 * opts:{depth(默认8), maxClass(默认2), out 排除区域, vp 视口过滤(默认true,只对视口内建树,false=全部),
 *       vm 纵向余量(默认3,上下各拓宽 N 个视口高), at 坐标锚定(center|x,y|相对比例,取该屏点最顶层元素),
...
async function tree(target, selector, opts = {}) {
  ...
  if (opts.at) { // 坐标锚定:用 elementFromPoint 解析屏幕点 → 树其"最近语义容器"
    ...
    sel = hit.selector;
  } else if (!sel) { // 无参 → 可见内容概览(视口内内容项:标题/作者/正文/统计,阅读序)
    const vis = await evaluate(target, VISIBLE_JS, 30000);
    const items = (vis && vis.items) || [];
    if (!items.length) return { ok: true, items: [] };
    const lines = items.map((it, i) =>
      `${i + 1}. [${it.tag}${it.cls ? '.' + it.cls : ''}] "${it.title}"`
      + (it.author || it.stats ? `\n    ${[it.author, it.stats].filter(Boolean).join('  ·  ')}` : '')
      + (it.body ? `\n    ${it.body}` : '')
    );
    return { ok: true, items, lines };
  }
  const r = await evaluate(target, buildTreeExpr(sel, opts.depth ?? 8, opts.maxClass, opts.out, opts.vp, opts.vm, opts.vis), 30000);
