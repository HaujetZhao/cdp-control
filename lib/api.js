...
const { SNAPSHOT_JS, buildTreeExpr, CLICK_JS, FILL_JS, FOCUS_JS, GET_FOCUS_JS, OUTLINE_JS, CONTENT_JS } = require('./scripts');
 ...
 * 感知主命令——唯一感知入口。默认整页 body,不做可见性判定(不筛视口、不查 computed style),
 * 整页结构一次给全,由脚本内 hasText/productive 过滤无文本壳子树控输出量。
 * opts.{selector|xpath} 可选:指定只建该区域(取第一个匹配),两者都传时 selector 优先。
 */
async function tree(target, opts = {}) {
  const r = await evaluate(target, buildTreeExpr(rootExprOf(opts)), 30000);
  if (!r?.ok) throw new Error(r?.err || 'tree 失败');
  return r;
}

/** 把 selector/xpath 选项翻译成页面侧求根元素的 JS 表达式;缺省返回 body。 */
function rootExprOf({ selector, xpath } = {}) {
 ...
 * 注:本串会被拼进页面 JS 模板串,故内部不能有反引号;xpath 经 JSON.stringify 安全内嵌。 */
function shadowXPathExpr(xpJson) {
  return `(() => {
  const xp = ${xpJson};
  const ctxs = [document];
