...
/**
 * 结构树:把 target 页面整棵 body 输出为紧凑简化 HTML 树(文本 + 结构)。
 * 感知主命令——唯一感知入口。无参数,不做可见性判定(不筛视口、不查 computed style),
 * 整页结构一次给全,由脚本内 hasText/productive 过滤无文本壳子树控输出量。
 */
async function tree(target) {
  const r = await evaluate(target, buildTreeExpr(), 30000);
