...
test('genSel: 祖先有 id 时锚定祖先 + 下方位置链(精确命中 el)', () => {
  ...
  assert.equal(genSel(li2), '#root > ul > li:nth-of-type(2)');
});
...
test('genSel: 祖先有 data-testid 锚定 + 下方链', () => {
  ...
  assert.equal(genSel(a), 'header[data-testid="bar"] > nav > a');
});
