...
- 注入侧 DOM 相关逻辑(如 tree-core 的 buildTree/fold 折叠(DOM 采集)、feedback 的顶层新增元素判定、find.ts 的 recoverRef 沿 parentRef 跳表自愈)依赖真实 DOM,靠浏览器实测验收(见 SKILL.md 用法),不写单测。`formatTree` 的纯变换分支(`·屏` view 标注、shadow host 占位/fold 优先)均已有单测锁定。
