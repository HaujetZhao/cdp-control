...
**find 命令(`src/inject/find-entry.ts`,类 uBlock `:has-text()`)**:解决"tree 输出严禁 grep,但 ref 易失效"的矛盾——按文本/selector 找元素登记新 ref,不必整页重 tree。两种匹配:`--text` 整页 DFS(复用 `tree-core` 的 `childrenOf` 穿透 shadow + `subtreeText` 取子树文本)搜"自身或后代文本含关键词"元素,**深度上限 `MAX_DEPTH=14` + 单节点文本截断 `TEXT_LIMIT=4000`** 防深层 shadow 爆炸,**命中即止**(不深入其子,避免父子重复占满结果);`--selector` 走 `findRoot`(支持 `>>>` shadow 链)。命中元素**追加**进 `__cdpRefs`(`{el, parentRef:null}`,不顶旧 ref),拿 ref 号后 `buildTree(el, {viewport:true})` 取根行,把根节点的 `ref` 标成分配号让 `formatTree` 自动输出 `[ref=N]`。`--ancestor` 命中后爬父到容器;`--all` 收集全部(默认首个)。入参 `FindCmdArgs{text?,selector?,ancestor?,all?}`(注意区别于操作定位用的 `FindArgs`,后者已占名)。Node 侧 `api.find`/`inject-loader.ts` 的 `findExpr`/CLI `find --text/--selector [--ancestor] [--all]`(每行输出 `[ref=N] <line>`)。
