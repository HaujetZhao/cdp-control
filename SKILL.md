...
| `xpath --xpath-file <file> [--target]` | **按 xpath 查元素(shadow 穿透,含分步诊断)**:打印全部命中(标签/文本/稳定 selector);未命中时打印**分步诊断**,精确指出断在哪一步、当时候选是谁——用于排查 DevTools 复制的路径为何不命中。路径必须经 `--xpath-file` 传(已删位置参数;行首 `//` 会被 shell 静默改成 `/`,内联必错) |
