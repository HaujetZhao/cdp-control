...
- **区域聚焦**(重要):页面常有固定结构(header/侧栏/底部)占满视口、把关心区挤掉/截断。用 `--in` 只看目标容器(`view --in ".Comments-container"` 只看评论区,阅读序、不混 header)、`--out` 排除噪音区(`view --out "header, aside"`)。tree 同理(`tree <sel> --out ".CommentItem-actions"` 忽略操作按钮)。**先 `view` 大致定位 → `--in` 聚焦目标区 → 需要层级再 `tree` 目标区**。
...
| `tree <selector> [--depth N] [--out <s1,s2>] [--margin N] [--full] [--target]` | **钻取该元素 DOM 子树**为紧凑层级树(过滤垃圾节点/属性/包装节点);默认**只对视口±1屏内元素建树**(有界输出),`--margin N` 上下各拓宽 N 个视口高,`--out` 排除子区域,`--full` 含全部视口外——从 view 拿到短 selector 后看内部层级结构 |
...
| `cdp.tree(target, selector, opts?)` | 对象,字符串,`{depth,maxClass,out,vp,vm}` | 钻取子树为紧凑层级树:`{ok, selector, lines}`;`opts.out` 排除子区域,`opts.vp` 视口过滤(默认 true,false=含视口外),`opts.vm` 纵向余量(视口高倍数,默认1) |
