...
- **内容反馈**:CLI 分块换行输出——`→ 页面变化 · 新增内容:`(重复块折叠标 `(重复 N 次,已折叠)`)、`→ 页面变化 · 文本变化:`(逐条 `旧 → 新`,过滤前后相同)。**文本变化报前后值**(点赞后数字 583→584 直接可见);点"显示评论"后评论进来,反馈里直接看得到。操作行附 `selector:`(唯一),后续对该元素操作优先用 selector 而非 ref,避免 ref 重渲染失效。
...
- **ref 失效是局部的**:反馈末尾 `→ 提示:` 提醒——某个小元素 ref 失效不代表全失效,用更大的容器 ref 筛局部(`tree --ref <容器ref>`)即可,别整页重 tree。
...
| `list` | 确保浏览器就绪(CDP 未起自动启动)并列出所有 page tab(含手动开的),先报 tab 总数 |
