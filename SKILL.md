...
- `list` 会自动确保浏览器就绪(CDP 未起则启动)并列出 tab,无需先 `ensure` 再 `list`。意外的页面变化多由用户手动操作引起,模型不必猜测。
...
- **ref 失效只在失败时提示**:操作因 ref 失效失败时,会报 `ref 失效: 可能元素已更新,可用更高层级的 ref 进行 tree,查看更新后的局部区域`——某个小元素 ref 失效不代表全失效,用更大的容器 ref 筛局部(`tree --ref <容器ref>`)即可,别整页重 tree。
